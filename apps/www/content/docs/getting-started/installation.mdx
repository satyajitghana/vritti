...
Add the Vritti registry to your project's `components.json`:

```json
{
  "registries": {
    "@vritti": {
      "url": "https://vritti-ui.vercel.app/r"
    }
  }
}
```
...
```bash
npx shadcn@latest add @vritti/[component-name]
```

For example, to add the blur-fade component:

```bash
npx shadcn@latest add @vritti/blur-fade
```
